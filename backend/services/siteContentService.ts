...
import { NotFoundError } from "../middlewears/error";
...
class SiteContentService {
		...
		if (!record) {
			record = await prisma.siteContent.create({
				data: {
					...
					contact: {
						create: { title: "", description: "", email: "" },
					},
					social: {
						create: { github: "", twitter: "", linkedin: "" },
					},
	...
	static async updateSocial(data: {
		github: string;
		twitter: string;
		linkedin: string;
	}) {
	...
	static async updateSkills(
		skillsData: Array<{ id?: string; title: string; description: string }>
	) {
		const content = await this.getSiteContentRecord();
